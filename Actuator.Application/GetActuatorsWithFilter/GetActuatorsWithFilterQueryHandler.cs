        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var actuators = await _actuatorRepository.GetActuatorsWithFilter(request.WorkOrderNumber, request.SerialNumber, request.PCBAUid, request.ItemNumber,
                request.ManufacturerNumber, request.ProductionDateCode, request.CommunicationProtocol, request.ArticleNumber, request.ConfigNo, request.Software, request.StartDate, request.EndDate);
